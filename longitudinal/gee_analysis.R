...
# Pacote: geepack; família gaussiana/binomial/poisson e correlações AR-1/EX/UN
...
#' # summary(fit$model)
#' # fit$coef_ci
gee_fit <- function(data, formula, id, family=gaussian(), corstr=c("ar1","exchangeable","independence","unstructured"), weights=NULL){
  ...
  id_vec <- data[[id]]
  model <- geepack::geeglm(formula=formula, id=id_vec, data=data, family=family, corstr=corstr, weights=weights)
